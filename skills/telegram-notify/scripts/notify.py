...
def resolve_state_dir(explicit_path=None):
    ...
    if xdg:
        return Path(xdg).expanduser() / "ai-skills" / "telegram"
    if sys.platform == "darwin":
        return Path.home() / "Library" / "Application Support" / "ai-skills" / "telegram"
    return Path.home() / ".local" / "state" / "ai-skills" / "telegram"
